...
def attack(args, img_path,num):
	...
	model = ML_GCN()
	targets = 0
	sample = np.array(Image.open(img_path).resize((224,224)).convert('RGB'))/255.

	# img_dct = cv2.dct(sample)

	tarimage = np.random.rand(448,448,+3)
	...
	image = np.concatenate([sample, np.zeros((224,8,3)), perturbed], axis = 1)
	# scipy.misc.imsave('{}/figs/{}-{}-{}.jpg'.format(data_model,
	# 	args.attack_type, args.constraint, img_path), image)
	# scipy.misc.imsave('{}/figs/{}-{}-{}.jpg'.format(data_model,
	# 	args.attack_type, args.constraint, img_path+'p'), perturbed)
	print(model.query_num)
	imageio.imwrite('./QEBA_data/ori{n}.png'.format(n=num), image)
	imageio.imwrite('./QEBA_data/{m}.png'.format(m=num), perturbed)
...
if __name__ == '__main__':
	...
	for i in range(25,99):
		if i < 10:
			attack(args, 'data/voc/VOCdevkit/VOC2007/JPEGImages/00000{j}.jpg'.format(j=i),i)
		elif i >= 10 and i<100:
			attack(args, 'data/voc/VOCdevkit/VOC2007/JPEGImages/0000{j}.jpg'.format(j=i), i)
		else:
			attack(args, 'data/voc/VOCdevkit/VOC2007/JPEGImages/000{j}.jpg'.format(j=i), i)
