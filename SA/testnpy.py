...
path = "/home/czj/jjr/square-attack-master/" #文件夹目录
npy= os.listdir("/home/czj/jjr/square-attack-master/npy") #得到文件夹下的所有文件名称
npy255= os.listdir("/home/czj/jjr/square-attack-master/npy255") #得到文件夹下的所有文件名称
x1 = []
x2 = []
xw= []
xr = []
...
#             (1, 448, 448, 3))
#         adv = np.array(
#             Image.open('adv3ws/{m}-p.png'.format(m=i)).resize((448, 448)).convert(
#                 'RGB')).reshape(
#             (1, 448, 448, 3))
...
#             (1, 448, 448, 3))
#         adv =np.array(
#             Image.open('advs/{m}.png'.format(m=i)).resize((448, 448)).convert(
#                 'RGB')).reshape(
#             (1, 448, 448, 3))
#         raw_array = np.array(img) / 255
#         adv_array = np.array(adv) / 255
...
for file in npy:  # 遍历文件夹
    if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
        test = np.load("/home/czj/jjr/square-attack-master/npy/" + file).reshape(-1)
...
for file in npy255:  # 遍历文件夹
    if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
        test_255 = np.load("/home/czj/jjr/square-attack-master/npy255/" + file).reshape(-1)
        xr.append(np.sqrt(np.mean(np.square(np.abs(test_255)))))
