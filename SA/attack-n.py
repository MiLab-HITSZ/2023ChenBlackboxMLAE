...
def square_attack_linf(model, x, y_target, eps, n_iters, p_init,name):
    ...
    for i_iter in range(n_iters - 1):
        ...
        if sucess:
            ...
            img.save('./advs10w/{m}.png'.format(m=name))
            break
    img = Image.fromarray(np.uint8(x_new[0]), 'RGB')
    img.save('./advs10w/{n}.png'.format(n=name))
    return n_queries, x_best
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eps', type=float, default=0.1, help='Radius of the Lp ball.')
    ...
    for i in range(10,99):
        if i < 10:
            x_test = (np.array(
                Image.open('data/voc/VOCdevkit/VOC2007/JPEGImages/00000{t}.jpg'.format(t=i)).resize((448, 448)).convert(
                    'RGB'))).reshape(
                1,
                448,
                448,
            ...
            x_test = (np.array(
                Image.open('data/voc/VOCdevkit/VOC2007/JPEGImages/0000{t}.jpg'.format(t=i)).resize((448, 448)).convert('RGB'))).reshape(
                1,
                448,
                448,
                3)
