...
parser.add_argument('--adv_method', default='ml_ba', type=str, metavar='N',
                    help='attack method: ml_de5, ml_cw, ml_rank1, ml_rank2, ml_deepfool, mla_lp:97')
parser.add_argument('--target_type', default='hide_single', type=str, metavar='N',
                    help='target method: hide_single')
parser.add_argument('--adv_file_path', default='../data/voc2007/files/VOC2007/classification_mlgcn_adv_hideall.csv', type=str, metavar='N',
...
def gen_adv_file(model, target_type, adv_file_path):
    ...
    for i in range(len(pred)):
        if (y[i] == pred[i]).all() and np.sum(y[i]) >= 2 and count < 100:
...
def evaluate_adv(state):
    ...
    adv_dataset = Voc2007Classification(args.data, 'mlgcn_adv_hidesingle',inp_name='../data/voc2007/voc_glove_word2vec.pkl') #
...
def main():
    ...
    adv_dataset = Voc2007Classification(args.data, 'mlgcn_adv_hideall', inp_name='../data/voc2007/voc_glove_word2vec.pkl')
    adv_dataset.transform = data_transforms
