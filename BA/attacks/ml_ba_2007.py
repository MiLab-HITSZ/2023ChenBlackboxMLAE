...
class MLBA(object):
    ...
    def generate_np(self, x_list, ori_loader, **kwargs):
        ...
        for i in range(len(x_list)):
            ...
            # hide all
            # imgg = x_list[i]
            # noise = np.random.normal(0., 1., size=imgg.size).reshape(imgg.shape[0], imgg.shape[1], imgg.shape[2])
            # # 加上噪声
            # imgg = imgg + noise
            # Adv = np.clip(imgg, 0., 1.)
...
def Genall( model, image, target,ori_loader,jump,yolonet,quit):
    img_dir = "../data/voc2007/VOCdevkit/VOC2007/JPEGImages/"
    imglist = os.listdir(img_dir)
    boxes = []
    target_label = np.argwhere(target > 0)
