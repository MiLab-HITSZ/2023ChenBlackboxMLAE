...
Time: 2024.1.11
Copyright: milab
For academic research only, commercial development is prohibited.
...
parser.add_argument('--adv_file_path', default='../data/voc2007/files/VOC2007/classification_mlgcn_adv_0_99_cut.csv', type=str, metavar='N',
                    help='all image names and their labels ready to attack')
parser.add_argument('--adv_save_x', default='../adv_save/mlgcn/voc2007/0_99_cut', type=str, metavar='N',
                    help='save adversiral examples')
parser.add_argument('--adv_begin_step', default=18, type=int, metavar='N',
...
def gen_adv_file(model, target_type, adv_file_path):
    ...
    for i in range(len(pred)):
        if (y[i] == pred[i]).all() and np.sum(y[i]) >= 2 and count>=0 and count<=99:
    ...
    # value is {-1,1}
    np.save('../adv_save/mlgcn/voc2007/y_target_0_99_cut.npy', y_target)
    np.save('../adv_save/mlgcn/voc2007/y_0_99_cut.npy', y)
...
def evaluate_adv(state):
    ...
    adv_file_list.sort(key=lambda x:int(x[:-8]))
    adv = []
    ...
    adv_dataset = Voc2007Classification(args.data, 'mlgcn_adv_0_99_cut',inp_name='../data/voc2007/voc_glove_word2vec.pkl') #
...
def main():
    ...
    ori_dataset = Voc2007Classification('../data/voc2007', 'val', inp_name='../data/voc2007/voc_glove_word2vec.pkl')
    ...
    adv_dataset = Voc2007Classification(args.data, 'mlgcn_adv_0_99_cut',
    ...
    # value is {-1,1}
    y_target = np.load('../adv_save/mlgcn/voc2007/y_target_0_99_cut.npy')
    y = np.load('../adv_save/mlgcn/voc2007/y_0_99_cut.npy')
