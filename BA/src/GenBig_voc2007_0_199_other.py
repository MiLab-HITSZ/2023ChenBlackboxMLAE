...
from src.Gen_model import AttackModel
...
parser.add_argument('--adv_file_path', default='../data/voc2007/files/VOC2007/classification_mlliw_adv_0_199_other.csv', type=str, metavar='N',
                    help='all image names and their labels ready to attack')
parser.add_argument('--adv_save_x', default='../adv_save/mlliw/voc2007/0_199_other', type=str, metavar='N',
...
def gen_adv_file(model, target_type, adv_file_path):
    ...
    for i in range(len(pred)):
        if (y[i] == pred[i]).all() and np.sum(y[i]) >= 2 and count>=0 and count<=199:
    ...
    # value is {-1,1}
    np.save('../adv_save/mlliw/voc2007/y_target_0_199_other.npy', y_target)
    np.save('../adv_save/mlliw/voc2007/y_0_199_other.npy', y)
...
def evaluate_adv(state,adv_method):
    model = state['model']
    y_target = state['y_target']

    adv_folder_path = os.path.join(args.adv_save_x, adv_method, 'tmp/')
    ...
    adv_dataset = Voc2007Classification(args.data, 'mlliw_adv_0_199_other')
    ...
    dataframe.to_excel('./unsucc_' + adv_method + '.xls')
    print('攻击不成功的为：')
    print(unsucc)
...
def main():
    ...
    for adv_method in [ "ml_deepfool","ml_rank1", "ml_rank2,ml_cw"]: #"ml_deepfool","ml_rank1", "ml_rank2",
        print(adv_method)
        init_log(os.path.join(args.adv_save_x, adv_method, args.target_type + '.log'))
        ...
        adv_dataset = Voc2007Classification(args.data, 'mlliw_adv_0_199_other') #zz注意！
        adv_dataset.transform = data_transforms
        ...
        # value is {-1,1}
        y_target = np.load('../adv_save/mlliw/voc2007/y_target_0_199_other.npy')
        y = np.load('../adv_save/mlliw/voc2007/y_0_199_other.npy')
        state = {'model': model,
                     ...
                     'y_target':y_target,
                     'y': y,
                     'adv_save_x': os.path.join(args.adv_save_x, adv_method, args.target_type + '.npy'),
                     'adv_begin_step': args.adv_begin_step
                     }
        # start attack
        attack_model = AttackModel(state)
        attack_model.attack()
        evaluate_adv(state,adv_method)
