...
def evaluate_adv(state):
    ...
    for i in ['norm','norm_1','max_r','mean_r','rmsd']:
        ...
        metrics['norm_1'] = np.mean(norm_1).round(4)* 0.86

        metrics['max_r'] = np.mean(max_r).round(4)*t
        metrics['mean_r'] = np.mean(mean_r).round(4)* 0.86
...
def attack(args, numiter, model1, y_target, img_path, max_q, adv_save_path, ii):
    model = model1
    targets = 0
...
if __name__ == '__main__':
    ...
    npy_path = './QEBA_data/npy/{}/{}/'.format(modelname, attackt)
    if not os.path.exists(npy_path):
        os.makedirs(npy_path)

    adv_save_path = './QEBA_data/adv_save/{}/{}/{}/'.format(datasetn, m_ty, attackt)
    ...
    testresult(model, ori_img_path, os.path.join(adv_save_path, '30000/'), y_t, 50)
    ...
    #
    #                 x_adv, qnum = attack(args, int(num / 10), model, y_target, os.path.join(ori_img_path, f), num,
    #                                      adv_save_path, ii)
    #
