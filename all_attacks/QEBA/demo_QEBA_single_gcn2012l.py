...
def attack(args, numiter,model1, img_path,tar_path):
    ...
    ori = ((model(torch.tensor((sample448 ).transpose(2,0,1)).float().unsqueeze(0).cuda())>0.5)+0).squeeze().cpu().numpy()
    print('ini label:',ori)
    ...
    sample112 = np.array(Image.open(img_path).resize((112, 112)).convert('RGB'))
...
if __name__ == '__main__':
    ...
    model = get_model('gcn2012').to(device)


    adv_save_path = './QEBA_data/adv_save/voc2012/mlgcn/hidesingle/'
    if not os.path.exists(adv_save_path):
        os.makedirs(adv_save_path)

    ori_448_path = './QEBA_data/single_ori_save/gcn_voc2012/'
    if not os.path.exists(ori_448_path):
        os.makedirs(ori_448_path)

    ini_adv_path = './hidesingle/GCN_2012_adv/'

    ori_folder_path = './hidesingle/ori_2012gcn/'
    ori_file_list = os.listdir(ori_folder_path)
