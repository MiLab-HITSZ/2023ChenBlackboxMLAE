...
def attack(args, numiter,model1, img_path,tar_path):
    model = model1
    targets = 0

    sample448 = np.array(Image.open(img_path).resize((448, 448)).convert('RGB')) / 255.

    t_sample = np.array(Image.open(tar_path).resize((448, 448)).convert('RGB')) / 255.
    targets = ((model(torch.tensor((t_sample).transpose(2, 0, 1)).float().unsqueeze(0).cuda()) > 0.5) + 0).cpu().numpy()
    ori = ((model(
        torch.tensor((sample448).transpose(2, 0, 1)).float().unsqueeze(0).cuda()) > 0.5) + 0).squeeze().cpu().numpy()
    print('ini label:', ori)
    print('target label:', targets)

    if np.all(ori==targets):
        print('wrong ini')
        return t_sample , 0

    sample1 = np.array(Image.open(img_path).resize((112, 112)).convert('RGB'))
...
if __name__ == '__main__':
    ...
    model = get_model('liw2012').to(device)


    adv_save_path = './QEBA_data/adv_save/voc2012/mlliw/hidesingle/'
    if not os.path.exists(adv_save_path):
        os.makedirs(adv_save_path)

    ori_448_path = './QEBA_data/single_ori_save/liw_voc2012/'
    if not os.path.exists(ori_448_path):
        os.makedirs(ori_448_path)

    ini_adv_path = './hidesingle/LIW_2012_adv/'

    ori_folder_path = './hidesingle/ori_2012liw/'
    ori_file_list = os.listdir(ori_folder_path)
    ...
    with torch.no_grad():
        for num in [ 3000,300]:
            ...
            for f in ori_file_list:
                ...
                test = Image.open(os.path.join(ori_folder_path,f)).resize((448, 448)).convert('RGB')
                test.save(os.path.join(ori_448_path, '{m}.png'.format(m=ii)))
                print('This is num {} pic\n'.format(ii))

                print(os.path.join(ori_folder_path,f),os.path.join(ini_adv_path,f))

                x_adv,qnum =attack(args,int(num/100), model, os.path.join(ori_folder_path,f),os.path.join(ini_adv_path,f))
