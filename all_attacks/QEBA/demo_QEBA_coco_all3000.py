...
if __name__ == '__main__':
    ...
    # model = ML_GCN2007()
    model = get_model('gcncoco').to(device)

    npy_path = './QEBA_data/npy/gcncoco/hideall/'

    if not os.path.exists(npy_path):
        os.makedirs(npy_path)

    adv_save_path = './QEBA_data/adv_save/coco/mlgcn/hideall/'
    if not os.path.exists(adv_save_path):
        os.makedirs(adv_save_path)


    ori_img_path = './cocoori/'
