...
def evaluate_adv(state):
    ...
    # 原始 # 按照2范数来选择
    adv_pred_match_target = (np.logical_and(np.all((adv_pred == y_target), axis=1), np.asarray(norm) < (77.6 * 1))) + 0
...
if __name__ == '__main__':
    ...
    npy_path = './npy/gcn2012/hideall/'

    if not os.path.exists(npy_path):
        os.makedirs(npy_path)

    adv_save_path = './adv_save/voc2012/mlgcn/hideall/'
    ...
    # testresult(model, ori_img_path, os.path.join(adv_save_path, '3000/'),y_t ,20)
