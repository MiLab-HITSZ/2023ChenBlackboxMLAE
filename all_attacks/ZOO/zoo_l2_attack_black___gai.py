...
def l2_attack(input, target, model, targeted, use_log, use_tanh, solver, reset_adam_after_found=True, abort_early=True,
              batch_size=1, max_iter=3800, const=1000000, confidence=0.0, early_stop_iters=2000, binary_search_steps=13,
    ...
    for step in range(binary_search_steps):
        ...
        if compare(bestscore, target.cpu().numpy()) and bestscore.all() != -1:
...
if __name__ == '__main__':
    ...
    #     (1, 448, 448, 3)) / 255. - 0.5
    inputs = sample = np.array(
        Image.open('data/voc/VOCdevkit/VOC2007/JPEGImages/000002.jpg').resize((448, 448)).convert('RGB')).reshape(
        (1, 448, 448, 3)) / 255. - 0.5
    ...
    print(adv[0].shape)
    print(adv)
    Image.fromarray(np.uint8((adv[0].reshape(448, 448, 3) + 0.5 ) * 255)).save('2.png')
    timeend = time.time()
