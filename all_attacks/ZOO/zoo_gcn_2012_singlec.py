...
def loss_run(input, target, model, modifier, use_tanh, use_log, targeted, confidence, const):
    ...
    output = model(pert_out.permute(0, 3, 1, 2).float()+0.5)
    # print(output)



    # print(pert_out.shape)
    # print(input.shape)
...
def l2_attack(input, target, model, targeted, use_log, use_tanh, solver, reset_adam_after_found=True, abort_early=True,
              batch_size=100, max_iter=50, const=10000000, confidence=0.0, early_stop_iters=1000, binary_search_steps=2,
              step_size=0.1, adam_beta1=0.9, adam_beta2=0.999):
    early_stop_iters = early_stop_iters if early_stop_iters != 0 else max_iter // 10

    ori_sample = input.transpose(0, 3, 1, 2)+0.5
    ...
    with torch.no_grad():
        for step in range(binary_search_steps):
            ...
            for iter in range(max_iter):
                ...
                for i in range(batch_size):
                    var[i * 2 + 1].reshape(-1)[indice[i]] += 0.1
                    #defult = 0.0001
                    var[i * 2 + 2].reshape(-1)[indice[i]] -= 0.1
                ...
                if solver == "adam":
                    move = coordinate_ADAM(losses, indice, grad, hess, batch_size, mt, vt, real_modifier_numpy, adam_epoch,
                    ...
                    out_best_attack+=real_modifier_numpy

                    out_best_attack = np.clip(out_best_attack,-0.5,0.5)
                ...
                pert_images = pert_images   + real_modifier_numpy
            ...
            if not compare(bestscore, target.squeeze().cpu().numpy()) and (bestscore== ori_label.cpu().numpy()).all():
...
if __name__ == '__main__':
    ...
    npy_path = './npy/gcn2012/hidesingle/'

    if not os.path.exists(npy_path):
        os.makedirs(npy_path)

    adv_save_path = './adv_save/voc2012/mlgcn/hidesingle/'
    if not os.path.exists(adv_save_path):
        os.makedirs(adv_save_path)

    ori_img_path = './ori_test_2012/'

    ori_448_path = './ori_save/voc2012/'
    ...
    with torch.no_grad():
        ...
        for f in adv_file_list:
            ...
            inputs = np.array(test).reshape((1, 448, 448, 3)) / 255. - 0.5
            ...
            imageio.imwrite(os.path.join(adv_save_path, '{n}/{m}_adv.png'.format(n=num, m=ii)),
                            np.uint8((adv[0].reshape(448, 448, 3) + 0.5) * 255))
