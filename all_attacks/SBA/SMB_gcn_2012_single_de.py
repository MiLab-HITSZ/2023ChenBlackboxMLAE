...
from simba_another import SimBA
...
parser.add_argument('--num_iters', type=int, default=800, help='maximum number of iterations, 0 for unlimited')
    ...
    for num_iters in [800,500,200]:
        ...
        for f in adv_file_list:
            if ii >100:
                ...
                adv,flag,nq = attacker.simba_single(images, y_target, num_iters=num_iters, epsilon=0.3, targeted=False)


                x_test = (np.array(test).transpose(2, 0, 1).astype('float32') / 255.).reshape(1, 3, 448, 448)

                # print(adv.cpu().numpy()[None].shape,type(adv.cpu().numpy()[None]),images.cpu().numpy().shape,type(images.cpu().numpy()))

                if flag == 1:
                    x_adv_suc = adv.cpu().numpy()[None]
                    x_ori_suc = images.cpu().numpy()
                    success = success + 1
                    # count += count_tem
                    norm1.append(np.linalg.norm((x_adv_suc - x_ori_suc).flatten(), ord=1))
                    norm2.append(np.linalg.norm((x_adv_suc - x_ori_suc).flatten(), ord=2))
                    maxr.append(np.max(np.abs(x_adv_suc - x_ori_suc), axis=(0,1, 2)))
                    meanr.append(np.mean(np.abs(x_adv_suc - x_ori_suc), axis=(0,1, 2)))
                    print('now success ave',np.mean(norm2).round(4), np.mean(norm1).round(4), np.mean(maxr).round(4), np.mean(meanr).round(4))

                if flag ==1:
                    save_image( adv.cpu().numpy()[None], ii, adv_save_path, nq)
                    save_image(x_test, ii, ori_save_path, nq)
