...
def hsja(model,
    ...
    global sample
    sample = sample_
    original_label = model.get_prob(sample)*0
...
def decision_function(model, images, params):
    ...
    if params['target_label'] is None:
        ...
        return np.all((((prob>0.5)+0) == params['original_label']), axis=1)+0
    else:
        return np.all((((prob>0.5)+0) == params['target_label']), axis=1)+0
...
def binary_search_batch(original_image, perturbed_images, model, params):
    ...
    prob = model.get_prob_(out_image.reshape(-1, 448, 448, 3))
    # print((prob>0.5)+0)
    if np.all((((prob>0.5)+0) == params['original_label']), axis=1):
        print("----success bi search----------",np.all((((prob>0.5)+0) == params['original_label']), axis=1))
