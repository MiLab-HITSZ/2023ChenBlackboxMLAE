...
def attack(args, img_path,num):
	...
	model = ML_GCN2007()
	targets = 0
	sample = np.array(Image.open(img_path).resize((448,448)).convert('RGB'))/255.
	...
	image = np.concatenate([sample, np.zeros((448,8,3)), perturbed], axis = 1)*255
	...
	imageio.imwrite('./advs10w/ori/{n}.png'.format(n=num), np.uint8(image))
	imageio.imwrite('./advs10w/{m}-p.png'.format(m=num),  np.uint8(perturbed*255))
