...
def get_args():
    ...
    parser.add_argument("--modelname", "-model", default="gcn2012", help="modelname")
    parser.add_argument("--attacktype", "-attack", default="hidesingle", help="hideall, hidesingle")
...
## single##
    parser.add_argument("--ini_path", "-ini", default="./hidesingle/GCN_2012_adv/", help="ini_path")
    parser.add_argument("--ori_folder", "-ori", default="./hidesingle/ori_2012gcn/", help="ori folder")
    parser.add_argument("--ori_folder_save", "-ori_save", default="./hidesingle/448/ori_2012gcn/", help="ori folder")

###
    # parser.add_argument("--ori_folder", "-ori", default="./adv_save/gcn2007/ori/", help="ori folder")

    parser.add_argument("--output_folder", "-out", default="./adv_save/gcn2012/", help="Output folder")
...
if __name__ == "__main__":
    ...
    for num in [300]:
        ...
        if num == 300:
            ...
            advs = f_attack(model, X, y,y_target,Ini.cuda(), **config0["run"])
        elif num ==3000:
            f_attack = SurFree(**config1["init"])
            advs = f_attack(model, X, y,y_target,Ini.cuda(), **config1["run"])
        elif num ==30000:
            f_attack = SurFree(**config2["init"])
            advs = f_attack(model, X, y, y_target, Ini.cuda(), **config2["run"])
        #######################有目标###################
