...
from utils.attack import _get_init_with_noise
...
class SurFree():
    ...
    def __call__(self, model, X, labels,tar, starting_points=None, **kwargs):
        ...
        self.labels = torch.where((labels - 0.5) > 0, 1, 0)
        self._model = model
        self.tar = tar
        # print('=============tattatatatatatatatatataaty',self.tar)

        # Get Starting Point
        self.best_advs = _get_init_with_noise(model, X, self.tar) if starting_points is None else starting_points
        self.X = X

        #判断  原始输入 样本 是不是 对抗
        is_adv = torch.sum((torch.where((model(X) - 0.5) > 0, 1, 0) != self.tar.cuda()), dim=1)==0

        self._images_finished = is_adv.cuda()
        #这里是判断X输入数据是不是对抗样本，如果是的话就选择X为起点，否则的话选择self.best_advs为算法起点
    ...
    def _is_adversarial(self, perturbed):
        ...
        is_adv = torch.sum((torch.where((self._model(perturbed) - 0.5) > 0, 1, 0) != self.tar.cuda()), dim=1) == 0
        indexes = []
    ...
    def _alpha_binary_search(self, function_evolution, lower):    
        ...
        iii = 0
        while mask_upper.any() and iii<20:
            # print(mask_upper)
            # print('3')
            iii+=1
