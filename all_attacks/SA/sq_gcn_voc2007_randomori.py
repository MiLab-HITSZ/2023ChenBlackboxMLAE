...
def square_attack_linf(model, x, y_target, eps, n_iters, p_init):
    ...
    for i_iter in range(n_iters - 1):
        ...
        # inimg = ( x_curr* 255).astype(np.uint8)
        # x_best_image = Image.fromarray(np.transpose(inimg.squeeze(0), (1, 2, 0)))
        # x_best_image.save('./sa_gener/x_curr_{}.png'.format(i_iter))
        #
        # inimg = (x_best_curr * 255).astype(np.uint8)
        # x_best_image = Image.fromarray(np.transpose(inimg.squeeze(0), (1, 2, 0)))
        # x_best_image.save('./sa_gener/x_best_curr_{}.png'.format(i_iter))
        ...
        if idx_improved:
            ...
            top_left =  (center_w, center_h)  # 左上角坐标
            bottom_right = (center_w + s, center_h + s)

            draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
            # 保存修改后的图像
            pil_image.save('./sa_box/box_{}.png'.format(i_iter))  # 保存为 PNG 格式

            if os.path.exists('./sa_box/box_2.png'):
                pil_image = Image.open('./sa_box/box_2.png')
                draw = ImageDraw.Draw(pil_image)
                top_left = (center_w, center_h)  # 左上角坐标
                bottom_right = (center_w + s, center_h + s)
                draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
                pil_image.save('./sa_box/box_2.png')  # 保存为 PNG 格式
