...
if __name__ == '__main__':
    ...
    adv_save_path = './adv_save/voc2007/mlliw/hidesingle/3w/'
    ...
    for f in adv_file_list:
        ...
        n_queries, x_adv = square_attack(model, x_test, y_target, args.eps, args.n_iter,
                                         args.p)
        end_time = time.time()
        round(end_time-start_time)

        print('Take {} S time'.format(round(end_time-start_time)))

        img = Image.fromarray(np.uint8(x_adv.squeeze() * 255).transpose(1, 2, 0), 'RGB')

        img.save(os.path.join(adv_save_path, '{m}_adv.png'.format(m=ii)))
