...
def mutate11( pop, F, img_width, img_height,iii):
    ...
    for i in range(len(pop)):
        ...
        if not os.path.exists('./de_box/mute{}.png'.format(iii)):
            pil_image =Image.open('x_adv.png')
            ...
            draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
            # 保存修改后的图像
            pil_image.save('./de_box/mute{}.png'.format(iii))  # 保存为 PNG 格式

        else:
            pil_image = Image.open('./de_box/mute{}.png'.format(iii))
            draw = ImageDraw.Draw(pil_image)
            top_left = (u1_x, u1_y)  # 左上角坐标
            bottom_right = (u1_x + l, u1_y + l)
            draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
...
def mutate_best(population,fitness, F,iter):
    new_population = []
...
#初始化50个不重复种群
    ...
    for gener in range(100):
        print('this is ',gener,'iter')
        if gener %10 ==0:
            print(population[0][2])
            population[:, 2] = int(population[0][2]*3/4)
            print(population[0][2])
        deltas_box = np.random.choice([-eps, eps], size=[3, 1, 1])
