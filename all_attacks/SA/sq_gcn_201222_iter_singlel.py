...
def evaluate_adv(state):
    ...
    #                    np.asarray(norm) < (77.6 * 10))  ,np.asarray(norm) >(0.00)) ) + 0
...
def square_attack_linf(model, x, y_target, eps, n_iters, p_init):
    ...
    _,x_p = get_loss(model, x, y_tar)
    print(x_p,y_tar,y_target)
    if get_decision(x_p, x_p*0) :
        print('fail ini',get_decision(x_p, x_p*0))
        return 1, x,0
    ...
    for i_iter in range(n_iters - 1):
        ...
        if sucess:
            ...
            return n_queries, x_best,1
    print('fail')
    return n_queries, x_best,0
...
if __name__ == '__main__':
    ...
    testresult(model, ori_img_path, os.path.join(adv_save_path, '800/'), y_t, 100)


    # #
    # for num in [200,500,800]:
    ...
    #             # metrics['max_r'] = np.mean(max_r).round(4)
    #             # metrics['mean_r'] = np.mean(mean_r).round(4)
    #             # metrics['rmsd'] = np.mean(norm).round(4)/6.0859
    #             # print(metrics)
    #
    #             qc = np.asarray(qc)
    #             print(np.sum(qc),sucn)
    #
    #
    #
    #             break
    ...
    #             y_target = y_t[ii-1]
    #
    #             n_queries, x_adv ,flag1= square_attack(model, x_test, y_target, args.eps, num,
    #                                              args.p)
    #             if flag1 ==1:
    #                 qc.extend(n_queries)
    #                 print(n_queries)
    ...
    #
    #             # if flag1==1:
    #             #     batch_r = (x_adv - x_test)
    #             #     batch_r_255 = ((x_adv - x_test / 2 + 0.5) * 255) - ((x_adv - x_test / 2 + 0.5) * 255)
    #             #     batch_norm = [np.linalg.norm(r.flatten()) for r in batch_r]
    #             #     batch_rmsd = [np.sqrt(np.mean(np.square(r))) for r in batch_r_255]
    #             #     norm.extend(batch_norm)
    #             #     rmsd.extend(batch_rmsd)
    #             #     norm_1.extend(np.sum(np.abs(x_adv - x_test), axis=(1, 2, 3)))
    #             #     max_r.extend(np.max(np.abs(x_adv - x_test), axis=(1, 2, 3)))
    #             #     mean_r.extend(np.mean(np.abs(x_adv - x_test), axis=(1, 2, 3)))
    #
