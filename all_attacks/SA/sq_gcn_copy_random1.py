...
def p_selection_box(p_init, it, n_iters):
    """ Piece-wise constant schedule for p (the fraction of pixels changed on every iteration). """
    it = int(it / n_iters * 10000)
...
def initialize_population(pop_size, image_size):
    """初始化种群，每个个体为两个随机坐标"""
    return np.random.randint(0, image_size, size=(pop_size, 4))  # 每个个体为 (x1, y1, x2, y2)


def fitness_function(pop, min_val, max_val,c,h,w,eps,x_c,x_b,d,y_tar):
    """计算适应度"""
    ...
    for individual in pop:
        ...
        width =  np.abs(individual[2] - individual[0])    # 这里假设 individual[0] 是左上角 x 坐标，individual[2] 是右下角 x 坐标
        height = np.abs(individual[3] - individual[1])  # individual[1] 是左上角 y 坐标，individual[3] 是右下角 y 坐标
        # 随机选择一个矩形区域进行扰动
        center_h = np.random.randint(0, h - height)
        center_w = np.random.randint(0, w - width)
        x_curr_window = x_curr[0, :, center_h:center_h + height, center_w:center_w + width]
        x_best_curr_window = x_best_curr[0, :, center_h:center_h + height, center_w:center_w + width]
        # prevent trying out a delta if it doesn't change x_curr (e.g. an overlapping patch)

        jishuqi = 0
        while np.sum(np.abs(np.clip(x_curr_window + deltas[0, :, center_h:center_h + height, center_w:center_w + width],
                                    min_val, max_val) - x_best_curr_window) < 10 ** -7) == c * height * width:
            jishuqi += 1
            # print('计数器：',jishuqi)
            deltas[0, :, center_h:center_h + height, center_w:center_w + width] = np.random.choice([-eps, eps],
                                                                                              size=[c, 1, 1])
            if jishuqi>10000:
                break
        ...
        # 这里的代码代表应用您提供的生成 x_new 的逻辑
        # 例如，使用前述代码的逻辑来更新 x_curr
        # 计算适应度
        loss ,_= get_loss(model, x_new, y_tar)  # 假设 loss_function 返回损失值
        fitness_scores.append(loss)


    return np.array(fitness_scores)

def mating_best1(pop, fitness, F):
    try:
        ...
        for i in range(len(pop)):
            ...
            mutation[i] = np.clip(mutation[i], 2, 446)
        return mutation
...
def mating_best(pop,fitness,F):
    ...
    for i in range(len(pop)):
        ...
        mutation[i] = pop[best] + F * (p2[i]-p3[i])
        mutation[i] = np.clip(mutation[i], 2,446)
    mutation = np.ceil(np.abs(mutation)).astype(int)
    return mutation
...
def mating(pop,F):
    ...
    mutation = np.ceil(np.abs(mutation)).astype(int)
    for i in range(len(pop)):
        mutation[i] = np.clip(mutation[i],2,224)
    return mutation

def select_next_generation(pop, fit_pop, off, fit_off):
    """选择下一代种群，选择适应度最小的个体"""

    # 将父代和子代合并
    combined_pop = np.vstack((pop, off))  # 合并父代和子代，形状为 (40, 4)

    # 合并适应度得分
    combined_fit = np.vstack((fit_pop, fit_off))  # 合并适应度得分，形状为 (40, 1)

    # 获取适应度最小的个体的索引
    best_indices = np.argsort(combined_fit.flatten())[:20]  # 选择适应度最小的20个个体

    # 返回新的种群和适应度
    new_pop = combined_pop[best_indices]  # 选出的20个个体，形状为 (20, 4)
    new_fit = combined_fit[best_indices]  # 对应的适应度分数，形状为 (20, 1)

    return new_pop, new_fit



def initialize_population11(pop_size, h, w):
    """初始化种群，确保能有效选择矩形区域"""
    population = np.zeros((pop_size, 4), dtype=np.int32)  # 使用 int32 节省内存

    for i in range(pop_size):
        x1 = np.random.randint(0, w)
        y1 = np.random.randint(0, h)
        width = np.random.randint(1, w - x1 + 1)
        height = np.random.randint(1, h - y1 + 1)
        population[i] = [x1, y1, x1 + width, y1 + height]

    return population

def mating_best22(pop, fitness, F):
    best = np.argmin(fitness)
    mutation = np.empty_like(pop)  # 使用 empty_like 减少内存分配

    p2 = np.random.permutation(pop)  # 随机打乱
    p3 = np.random.permutation(pop)

    for i in range(len(pop)):
        mutation[i] = pop[best] + F * (p2[i] - p3[i])
        mutation[i] = np.clip(mutation[i], 0, 448).astype(np.int32)  # 确保类型一致并裁剪

    return mutation

def select_next_generation(pop, fit_pop, off, fit_off):
    combined_pop = np.vstack((pop, off))
    combined_fit = np.vstack((fit_pop, fit_off))

    best_indices = np.argsort(combined_fit.flatten())[:20]
    new_pop = combined_pop[best_indices]  # 选出的20个个体
    new_fit = combined_fit[best_indices]   # 对应的适应度分数
...
def DEE(pop_size,gener,min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar):
    print('DE_begin')
    generation_save = np.zeros((10000,))
    pop = initialize_population(pop_size, 224)

    # pop = initialize_population11(pop_size, h, w)

    # print(pop)
    fitness_scores = fitness_function(pop, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)
    # print('初始的fitness',fitness_scores)

    count = 0
    fitmin = np.min(fitness_scores)
    # print('最小fitness', fitmin)
    generation_save[count] = fitmin
    F = 0.5
    if (len(np.where(fitness_scores == 0)[0]) == 0):
        while (count < gener):
            count += 1
            # off = mating_best1(pop, fitness_scores, F)
            off = mating(pop, F)
            # print('hello?')


            off_fitness = fitness_function(off, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)

            pop, fitness_scores = select_next_generation(pop, fitness_scores, off, off_fitness)

            fitmin = np.min(fitness_scores)
            generation_save[count] = fitmin
            print('当前最佳个体:', pop[np.argmin(fitness_scores)])

    print('历代', generation_save[:count + 1])
    print('最佳个体:', pop[np.argmin(fitness_scores)])
    return generation_save[:count + 1], pop[np.argmin(fitness_scores)]
...
def square_attack_linf(model, x, y_target, eps, n_iters, p_init):
    ...
    for i_iter in range(n_iters - 1):
        print('This is num {}'.format(i_iter))
        idx_to_fool = [0]
        ...
        # p = p_selection_box(i_iter, n_iters)
        width = 0  # 这里假设 individual[0] 是左上角 x 坐标，individual[2] 是右下角 x 坐标
        height = 0

        if int(i_iter / n_iters * 10000)==0:
            a,b= DEE(20,10, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)
            i_iter+=200

            x1, y1, x2, y2 = b
            s = int(round(np.sqrt( np.abs(x2 - x1)*np.abs(y2 - y1) )))
            print(s)  # individual[1] 是左上角 y 坐标，individual[3] 是右下角 y 坐标
        elif int(i_iter / n_iters * 10000)==500:
            a,b= DEE(20,10, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)
            i_iter += 200
            x1, y1, x2, y2 = b
            s = int(round(np.sqrt(np.abs(x2 - x1) * np.abs(y2 - y1))))
            print(s)
        elif int(i_iter / n_iters * 10000)==1000:
            a,b= DEE(20,10, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)
            i_iter += 200
            x1, y1, x2, y2 = b
            s = int(round(np.sqrt(np.abs(x2 - x1) * np.abs(y2 - y1))))
            print(s)
        elif int(i_iter / n_iters * 10000)==2000:
            a,b= DEE(20,10, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)
            i_iter += 200
            x1, y1, x2, y2 = b
            s = int(round(np.sqrt(np.abs(x2 - x1) * np.abs(y2 - y1))))
            print(s)
        elif int(i_iter / n_iters * 10000)==4000:
            a,b= DEE(20,10, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)
            i_iter += 200
            x1, y1, x2, y2 = b
            s = int(round(np.sqrt(np.abs(x2 - x1) * np.abs(y2 - y1))))
            print(s)
        elif int(i_iter / n_iters * 10000)==6000:
            a,b= DEE(20,10, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)
            i_iter += 200
            x1, y1, x2, y2 = b
            s = int(round(np.sqrt(np.abs(x2 - x1) * np.abs(y2 - y1))))
            print(s)
        elif int(i_iter / n_iters * 10000)==8000:
            a,b= DEE(20,10, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)
            i_iter += 200
            x1, y1, x2, y2 = b
            s = int(round(np.sqrt(np.abs(x2 - x1) * np.abs(y2 - y1))))
        ...
        x_new = np.clip(x_curr + deltas, min_val, max_val)
        x_new1 =  np.clip(x_curr - deltas, min_val, max_val)
        ...
        #     jijiji = 0
        #     while np.sum(
        #             np.abs(np.clip(x_curr_window + deltas[0, :, center_h:center_h + height, center_w:center_w + width],
        #                            min_val, max_val) - x_best_curr_window) < 10 ** -7) == c * height * width:
        #         deltas[0, :, center_h:center_h + height, center_w:center_w + width] = np.random.choice([-eps, eps],
        #                                                                                                size=[c, 1, 1])
        #         jijiji+=1
        ...
        # loss1 , pred_label1 = get_loss(model, x_new1, y_tar)
        # print(loss, loss1)
        # if loss1<loss:
        #     x_new = x_new1
        ...
        arr = np.append(arr, loss)

        # print('iter {} loss is {}',n_queries,loss)
        ...
        if loss ==0:
            ...
            return n_queries, x_new,sssss,arr
        ...
        # if loss < 0.3:
        #
        #     r, count_tem, fit = DE(25, 200, 448 * 448 * 3, model,np.squeeze(x) ,np.squeeze(x_best),
        #                            y_tar, 0.05, 1, gradient=None)
        #     # print(fit.shape)
        #     combined = np.concatenate((arr, fit))
        #     # print(combined.shape)
        #     x_best[idx_to_fool] = np.clip(x_best[idx_to_fool] + np.reshape(r, x_best[idx_to_fool].shape[1:]) * 0.05, 0, 1)
        #     return n_queries, x_best, 1, combined


    return n_queries, x_best,sssss,arr
    ...
    for num in [5000]:
        ...
        for f in adv_file_list:
            ...
            if ii>0:
                ...
                # 保存图形
                plt.savefig('juxing_boxs_de224_{}.png'.format(ii), dpi=300, bbox_inches='tight')  # 保存为 PNG 文件
                plt.show()  # 显示图形
