...
def mating_best(pop,fitness,F):
    ...
    for i in range(len(pop)):
        print('2222')
        mutation[i] = pop[best] + F * (p2[i]-p3[i])
        mutation[i] = np.clip(mutation[i], 0,448)
    print('裁剪',mutation)
    return mutation
...
def DEE(pop_size,gener,min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar):
    ...
    pop = initialize_population(pop_size, 448)
...
def square_attack_linf(model, x, y_target, eps, n_iters, p_init):
    ...
    for i_iter in range(n_iters - 1):
        ...
        if i_iter ==0:
            a,b= DEE(50,10, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)
            print('lidai:',a)
            print('zuijia ge ti :',b)

            x1, y1, x2, y2 = b

            width = np.abs(x2 - x1)  # 这里假设 individual[0] 是左上角 x 坐标，individual[2] 是右下角 x 坐标
            height = np.abs(y2 - y1)  # individual[1] 是左上角 y 坐标，individual[3] 是右下角 y 坐标
        elif i_iter == 50:
            a, b = DEE(50, 10, min_val, max_val, c, h, w, eps, x_curr, x_best_curr, deltas, y_tar)
            print('lidai:', a)
            print('zuijia ge ti :', b)
    ...
    for num in [5000]:
        ...
        for f in adv_file_list:
            ...
            if ii>0:
                ...
                # 保存图形
                plt.savefig('juxing{}.png'.format(ii), dpi=300, bbox_inches='tight')  # 保存为 PNG 文件
                plt.show()  # 显示图形
