...
def evalue(population, x_ori, x_adv_curr,y_tar, box_size,fitness):
    for i_img in range(x_adv_curr.shape[0]):
        for i, (x, y,l,deltas_box) in enumerate(population):
            # 提取窗口
            x_curr_window = x_ori[i_img, :, x:x+l, y:y+l]
            x_best_curr_window = x_adv_curr[i_img, :, x:x+l, y:y+l]
            deltas = x_adv_curr - x_ori
            # while np.sum(
            #         np.abs(np.clip(x_curr_window + deltas[i_img, :, x:x+l, y:y+l],
            #                        0, 1) - x_best_curr_window) < 10 ** -7) == 3 * box_size * box_size:

            deltas[i_img, :, x:x+l, y:y+l] = deltas_box

            # 更新样本
            x_update = np.clip(x_ori[i_img] + deltas, 0, 1)

            ini_img = (x_update * 255).astype(np.uint8)
            x_best_image = Image.fromarray(np.transpose(ini_img.squeeze(0), (1, 2, 0)))
            x_best_image.save('./update/x_update{}.png'.format(i))
...
def mutate11( pop, F, img_width, img_height,iii):
    mutated_individual = pop.copy()
    for i in range(len(pop)):
        x1, y1, l,db1 = pop[i]

        # 随机选择两个不同的个体
        idx2, idx3 = np.random.choice(len(pop), 2, replace=False)
        x2, y2, _,db2 = pop[idx2]
        x3, y3, _,db3= pop[idx3]

        # 计算变异后的坐标
        u1_x = x1 + F * (x2 - x3)
        u1_y = y1 + F * (y2 - y3)

        u1db = np.where(np.sign(db2) == np.sign(db3), db2, np.random.choice([-0.05, 0.05], size=db2.shape))
        # 确保变异后的左上角顶点在图像范围内
        u1_x = int(np.clip(np.abs(u1_x), 0, img_width - l))
        u1_y = int(np.clip(np.abs(u1_y), 0, img_height - l))

        mutated_individual[i] = (u1_x, u1_y, l,u1db)

        if not os.path.exists('mute{}.png'.format(iii)):

            inimg = (np.zeros((1, 3, 448, 448)) +125).astype(np.uint8)
            pil_image = Image.fromarray(np.transpose(inimg.squeeze(0), (1, 2, 0)))
            # 创建可绘制对象
            draw = ImageDraw.Draw(pil_image)
            top_left = (u1_x, u1_y)  # 左上角坐标
            bottom_right = (u1_x + l, u1_y + l)
            draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
            # 保存修改后的图像
            pil_image.save('mute{}.png'.format(iii))  # 保存为 PNG 格式

        else:
            pil_image = Image.open('mute{}.png'.format(iii))
            draw = ImageDraw.Draw(pil_image)
            top_left = (u1_x, u1_y)  # 左上角坐标
            bottom_right = (u1_x + l, u1_y + l)
            draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
...
def mutate_best( pop, pop_fit,F, img_width, img_height,iii):
    best_indi = np.argmin(pop_fit)
    mutated_individual = pop.copy()
    for i in range(len(pop)):
        x1, y1, l,db1 = pop[best_indi]
...
def mutate_best_half(pop, pop_fit, F, img_width, img_height, iii):
    # 找到适应度最好的 25 个个体
    sorted_indices = np.argsort(pop_fit)  # 从小到大排序
    best_indices = sorted_indices[:25]  # 获取前 25 个个体的索引
    worst_indices = sorted_indices[25:]  # 获取后 25 个个体的索引

    mutated_individual = pop.copy()

    # 处理最好的 25 个个体
    for i in best_indices:
        x_center, y_center, l, db = pop[i]

        # 根据 db 值生成新个体的位置
        offset_x = np.random.uniform(-l, l)
        offset_y = np.random.uniform(-l, l)

        # 计算新个体的坐标
        u1_x = int(np.clip(x_center + offset_x, 0, img_width - l))
        u1_y = int(np.clip(y_center + offset_y, 0, img_height - l))

        u1db = np.random.choice([-0.05, 0.05], size=db.shape)
        ...
        if not os.path.exists('mute{}.png'.format(iii)):
            ...
            bottom_right = (u1_x + l, u1_y + l)
            draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
            pil_image.save('mute{}.png'.format(iii))

    # 处理剩余的 25 个个体使用 DE/RAND 算子
    for i in worst_indices:
        # 随机选择三个不同的个体
        idx1, idx2, idx3 = np.random.choice(len(pop), 3, replace=False)

        x1, y1, l1, db1 = pop[idx1]
        x2, y2, l2, db2 = pop[idx2]
        x3, y3, l3, db3 = pop[idx3]
        ...
        # 确保变异后的左上角顶点在图像范围内
        u1_x = int(np.clip(np.abs(u1_x), 0, img_width - l1))
        u1_y = int(np.clip(np.abs(u1_y), 0, img_height - l1))
        u1db = np.where(np.sign(db2) == np.sign(db3), db2, np.random.choice([-0.05, 0.05], size=db2.shape))

        mutated_individual[i] = (u1_x, u1_y, l1, u1db)
...
def mutate_best_half_overlap(pop, pop_fit, F, img_width, img_height, iii):
    # 找到适应度最好的 25 个个体
    sorted_indices = np.argsort(pop_fit)
    best_indices = sorted_indices[:25]
    worst_indices = sorted_indices[25:]
    ...
    for i in best_indices:
        ...
        while overlap:
            ...
            # 检查新个体是否与现有个体重叠
            new_individual = (u1_x, u1_y, l, u1db)
            overlap = any(np.array_equal(new_individual, pop[j]) for j in range(len(pop)))

        mutated_individual[i] = new_individual
        if not os.path.exists('mute{}.png'.format(iii)):
...
def mutatesssss(population,fitness, F,iter):
    new_population = []
    for i in range(len(population)):
        ...
        # 变异
        box = population[np.argmin(fitness)]
        new_box = (
            box[0] + F * (population[r1][0] - population[r2][0]),
            box[1] + F * (population[r1][1] - population[r2][1]),
            box[2] + F * (population[r1][2] - population[r2][2]),
            box[3] + F * (population[r1][3] - population[r2][3])
        )
        # print('new_box',new_box)
        # 确保新框在图像内
        # new_box = (
        #     int(max(0, min(image_width - box_size, new_box[0]))),
        #     int(max(0, min(image_height - box_size, new_box[1]))),
        #     int(max(0, min(image_width, new_box[2]))),
        #     int(max(0, min(image_height, new_box[3])))
        # )
        ...
        # print('new_box', new_box)
        new_population.append(new_box)
        if i ==0:
            image = np.zeros((448, 448, 3), dtype=np.uint8) + 125.

            # 给定两个对角顶点的坐标
            top_left = (new_box[0], new_box[1])  # 左上角坐标
            bottom_right = (new_box[2], new_box[3])  # 右下角坐标

            # 绘制矩形框
            color = (255, 0, 0)  # 矩形的颜色（蓝色）
            thickness = 2  # 矩形边框的厚度

            cv2.rectangle(image, top_left, bottom_right, color, thickness)
            cv2.imwrite('outputnew_image{}.jpg'.format(iter), image)
        elif i!=0:
            image = cv2.imread('outputnew_image{}.jpg'.format(iter))
            top_left = (new_box[0], new_box[1])  # 左上角坐标
            bottom_right = (new_box[2], new_box[3])  # 右下角坐标
...
def selectpop1(pop, pop_fitness, off, off_fitness):
    pop_new = np.copy(pop)
    pop_new_fitness = np.copy(pop_fitness)

    for i in range(len(pop)):
        if pop_fitness[i] < off_fitness[i]:
            pop_new[i] = pop[i]
            pop_new_fitness[i] = pop_fitness[i]
        else:
            pop_new[i] = off[i]
            pop_new_fitness[i] = off_fitness[i]
    return pop_new, pop_new_fitness
...
#初始化50个不重复种群
    ...
    pop_fit = evalue(population, x_ori, x_adv_curr,y_tar, box_size,fitness)
    ...
    # 找到最小元素的索引
    indices = np.argsort(pop_fit)[:index]
    change_pop = population[indices]
    for i, (xc, yc, lc, db) in enumerate(change_pop):
        if not os.path.exists('output111_image.png'):
            inimg = (x_ori * 255).astype(np.uint8)
            pil_image = Image.fromarray(np.transpose(inimg.squeeze(0), (1, 2, 0)))
            # 创建可绘制对象
            draw = ImageDraw.Draw(pil_image)
            top_left = (xc, yc)  # 左上角坐标
            bottom_right = (xc + lc, yc + lc)
            draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
            # 保存修改后的图像
            pil_image.save('output111_image.png')  # 保存为 PNG 格式

        else:
            pil_image = Image.open('output111_image.png')
            draw = ImageDraw.Draw(pil_image)
            top_left = (xc, yc)  # 左上角坐标
            bottom_right = (xc + lc, yc + lc)
            draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
    ...
    loss_x_change, _ = get_loss(model, x_change, y_tar)

    print('loss_x_adv',loss_min)
    print('loss_x_ori', loss_x_change)
    ...
    for i, (xc, yc,lc,db) in enumerate(change_pop):
        ...
        # 提取窗口
        deltas = x_adv_curr - x_ori
        x_c_window = x_ori[0, :, xc:xc+lc, yc:yc+lc]
        x_b_window = x_adv_curr[0, :, xc:xc+lc, yc:yc+lc]
        while np.sum(
                np.abs(np.clip(x_c_window + deltas[0, :, xc:xc+lc, yc:yc+lc],
                               0, 1) - x_b_window) < 10 ** -7) == 3 * box_size * box_size:
            deltas[0, :, xc:xc+lc, yc:yc+lc] = db
        # 更新样本
        x_change_new = np.clip(x_ori + deltas, 0, 1)
        loss_x_update, _ = get_loss(model, x_change_new, y_tar)

        n_queries+=1

        if loss_x_update < loss_min:
            x_adv_curr = x_change_new

            imgggx = (x_adv_curr * 255).astype(np.uint8)
            x_best_image = Image.fromarray(np.transpose(imgggx.squeeze(0), (1, 2, 0)))
            x_best_image.save('./diejia/x_adv_curr{}.png'.format(i))
            loss_min = loss_x_update
            print('update loss_min',loss_min)

    loss_x_final, _ = get_loss(model, x_adv_curr, y_tar)
    print('x_adv_curr_fit',loss_x_final)
    ...
    for gener in range(50):
        print('this is ',gener,'iter')


        if gener %10 ==0 or flag==0:
            print(population[0][2])
            population[:, 2] = int(np.ceil(population[0, 2] * 4 / 5))
            print(population[0][2])
        deltas_box = np.random.choice([-eps, eps], size=[3, 1, 1])
        # off = mutate_best(population, pop_fit,0.5,i)
        # off = mutate(population,  0.5, i)


        # off = mutate11(population, 0.5, 448,448,gener)

        off = mutate_best_half_overlap(population,pop_fit, 0.5, 448,448,gener)


        off_fit = evalue(population, x_ori, x_adv_curr,y_tar, box_size,fitness)

        n_queries+=50

        # population, pop_fit = selectpop(population, pop_fit, off, off_fit)
        population, pop_fit = selectpop1(population, pop_fit, off, off_fit)
        print('xuan ze hou',pop_fit)



        # final_boxes = remove_overlapping_boxes(population)
        # for i, (xc, yc, lc, db) in enumerate(final_boxes):
        #     if not os.path.exists('remove{}.png'.format(gener)):
        #         inimg = (x_ori * 255).astype(np.uint8)
        #         pil_image = Image.fromarray(np.transpose(inimg.squeeze(0), (1, 2, 0)))
        #         # 创建可绘制对象
        #         draw = ImageDraw.Draw(pil_image)
        ...
        #         draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
        #         # 保存修改后的图像
        #         pil_image.save('remove{}.png'.format(gener))  # 保存为 PNG 格式
        #
        #     else:
        #         pil_image = Image.open('remove{}.png'.format(gener))
        #         draw = ImageDraw.Draw(pil_image)
        #         top_left = (xc, yc)  # 左上角坐标
        #         bottom_right = (xc + lc, yc + lc)
        #         draw.rectangle([top_left, bottom_right], outline="yellow", width=2)
        ...
        #
        #
        # print('final off pop',pop_fit[np.argmin(pop_fit)])
        ...
        indices = np.argsort(pop_fit)[:index]
        change_pop = population[indices]
        # print('截断的pop',change_pop.shape)
        x_change = x_ori.copy()
        #这里是原图

        # print('目前为止的loss', loss_min)

        count_update = 0

        for j, (xc, yc, lc,db) in enumerate(change_pop):
            ...
            if loss_x_update < loss_min:
                ...
                x_adv_curr = x_change_new
                loss_min = loss_x_update
                print('update loss_min', j, loss_min, count_update)
                imgggx_best_curr = (x_adv_curr * 255).astype(np.uint8)
                imgggx_best_curr_image = Image.fromarray(np.transpose(imgggx_best_curr.squeeze(0), (1, 2, 0)))
                imgggx_best_curr_image.save('./diedai/xadv_{}.png'.format(j))
                arr = np.append(arr, loss_min)
        if count_update >4:
            flag =1
        else:
            flag = 0
    ...
    for num in [10000]:
        ...
        for f in adv_file_list:
            ...
            if ii>0:
                ...
                # 创建横坐标（元素索引）
                indices = np.arange(len(filtered_data))  # 索引从 0 到 999

                # 绘制图形
                plt.figure(figsize=(10, 5))  # 设置图形大小
                plt.plot(indices, filtered_data, label='Random Values')  # 绘制随机值
                ...
                # plt.xticks(np.arange(0, len(data), 10)) # 设置 x 轴坐标以 10 为间隔显示
                # plt.yticks(data[::10])
