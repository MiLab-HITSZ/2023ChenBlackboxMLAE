...
def reflect_image_values(image):
    # 找到超出[0, 1]范围的像素值
    below_zero = image < 0
    above_one = image > 1
    # 对超出范围的像素值进行反射操作
    image[below_zero] = -image[below_zero]
    image[above_one] = 2 - image[above_one]
    return image

def evaluate_population(population, x, y_tar,fitness):
    with torch.no_grad():
        adv_ = np.tile(x, (len(population), 1, 1, 1)) + np.reshape(population, (len(population),) + x.squeeze().shape) *0.05
        # adv = reflect_image_values(adv_)
        adv = np.clip(adv_, 0, 1)

        # 注意这里这个adv_img是添加完大扰动后的原始图像了，adv就是添加完大扰动后再减去小扰动生成的对抗样本
        if torch.cuda.is_available():
            predict = model(torch.tensor(adv, dtype=torch.float32).cuda()).cpu()
        else:
            predict = model(torch.tensor(adv, dtype=torch.float32))

    p = np.copy(np.asarray(predict))
    q = np.zeros(p.shape) + 0.5
    fit = p - q
    # print(fit)
    fit[:, y_tar] = -fit[:, y_tar]
...
#
# def mutate_best(population, best_individual, F=0.5):
#     mutated_population = np.copy(population)
#
#     # 随机选择两个个体x1和x2
#     idx_x1, idx_x2 = np.random.choice(len(population), 2, replace=False)
#
#     # 对每个个体进行变异
#     for i in range(len(population)):
#         x1 = population[idx_x1]
#         x2 = population[idx_x2]
#
#         mutated_population[i] = best_individual + F * (x1 - x2)
#
...
def initialize_population(pop_size, gene_length, sparsity_ratio=1):
    if sparsity_ratio == 1:
        population = np.random.uniform(-1, 1, (pop_size, gene_length))
    else:
        population = np.zeros((pop_size, gene_length))
        num_nonzero_elements = int(gene_length * sparsity_ratio)
        nonzero_indices = np.random.choice(gene_length, num_nonzero_elements, replace=False)
        for i in range(pop_size):
            individual = np.random.uniform(-1, 1, num_nonzero_elements)
            population[i, nonzero_indices] = individual

    return population


def selectpop(pop, pop_fitness, off, off_fitness):
    pop_new = np.copy(pop)
...
def complement(pop,fitness,fit,x, y_tar, pop_fit) :
    popnew = pop.copy()
    ...
    off_fitness_new, off_fit_new = evaluate_population(popnew, x, y_tar, pop_fit)
...
def square_attack_linf(model, x, y_target, eps, n_iters, p_init):
    ...
    np.random.seed(998)  # important to leave it here as well
    sssss = 0
...
#初始化50个不重复种群
    ...
    population = initialize_population(50, 602112, sparsity_ratio=1)
    print(population.shape)
    print(np.max(population), np.min(population))
    ...
    fitness,fit = evaluate_population(population, x, y_tar,fitness)
    eval_count += 50

    print(fitness)

    pop_fit = fitness.copy()
    ...
    if (len(np.where(fitness == 0)[0]) == 0):
        while (eval_count < 10000):
            print('this is ', count, 'iter')
            gama = eval_count / (100 * 100)
            print(gama)
            count += 1
            off = mating_best(population,pop_fit,0.5)
            off_fitness, off_fit = evaluate_population(off, x, y_tar, pop_fit)
            eval_count += 50
            population, pop_fit, fit = select(population, pop_fit, fit, off, off_fitness, off_fit)
            population, pop_fit, fit = complement(population, pop_fit, fit, x, y_tar, pop_fit)
            fitmin = np.min(fitness)
            generation_save[count] = fitmin
            if (len(np.where(fitness == 0)[0]) != 0):
                break
    if (len(np.where(fitness == 0)[0]) != 0):
        return population[np.where(fitness == 0)[0][0]], eval_count, generation_save[:count + 1]
    else:
        return population[0], eval_count / 100, generation_save[:count + 1]
