...
path = "/home/czj/jjr/ZOO_Attack_PyTorch-main/" #文件夹目录
npy= os.listdir("/home/czj/jjr/ZOO_Attack_PyTorch-main/npy") #得到文件夹下的所有文件名称
npy255= os.listdir("/home/czj/jjr/ZOO_Attack_PyTorch-main/npy255") #得到文件夹下的所有文件名称
x1 = []
x2 = []
xw= []
xr = []
...
#             (1, 448, 448, 3))
#         adv = np.array(
#             Image.open('advs3w/{m}.png'.format(m=i)).resize((448, 448)).convert(
#                 'RGB')).reshape(
#             (1, 448, 448, 3))
...
for file in npy:  # 遍历文件夹
    if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
        test = np.load("/home/czj/jjr/ZOO_Attack_PyTorch-main/npy/" + file).reshape(-1)
        mean.append(np.mean(np.absolute(test)))
...
for file in npy255:  # 遍历文件夹
    if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
        test_255 = np.load("/home/czj/jjr/ZOO_Attack_PyTorch-main/npy255/" + file).reshape(-1)
