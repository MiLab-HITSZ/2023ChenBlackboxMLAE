...
def l2_attack(input, target, model, targeted, use_log, use_tanh, solver, reset_adam_after_found=True, abort_early=True,
              batch_size=1, max_iter=40000, const=10, confidence=0.0, early_stop_iters=100, binary_search_steps=6,
...
if __name__ == '__main__':
    ...
    for i in range(10,99):
        ...
        print(adv[0].shape)
        print(adv)
        Image.fromarray(np.uint8((adv[0].reshape(448, 448, 3) + 0.5 ) * 255)).save('./advs10w/{m}.png'.format(m=i))
        timeend = time.time()
