...
images = torch.tensor((np.array(
    Image.open('data/voc/VOCdevkit/VOC2007/JPEGImages/000011.jpg').resize((448, 448)).convert('RGB'))/255.).reshape(1,
                                                                                                               448,
                                                                                                               448,
                                                                                                               3))


labels = model.get_prob_(images).cpu().reshape((1, 20))

y_test_onehot = np.zeros([len(labels), 20], dtype=float)
...
for i in range(N):
    ...
    labels_batch = y_test_onehot[(i * args.batch_size):upper]
    ...
    img = Image.fromarray(np.uint8(adv*255).reshape(448, 448, 3), 'RGB')
    img.save('advs/11.png')
    if i == 0:
        all_adv = adv
        all_probs = probs
        all_succs = succs
